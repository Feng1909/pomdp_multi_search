...
class MosViz:
    ...
    def update(self, robot_id, action, observation, viz_observation, belief):
        ...
        if type(robot_id) != type(list()):
            ...
            for robot in robot_id:
                self._last_action[robot] = action
                self._last_observation[robot] = observation
                self._last_viz_observation[robot] = viz_observation
                self._last_belief[robot] = belief
    ...
    def on_render(self):
        ...
        for rx, ry, rth in self._env.state.pose(robot_id):
