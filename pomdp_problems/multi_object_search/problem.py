...
class MosOOPOMDP(pomdp_py.OOPOMDP):
    ...
    def __init__(self, robot_id, env=None, grid_map=None,
        ...
        if type(robot_id) == int:
            robot_id = robot_id
        elif type(robot_id) == list:
            robot_id = [interpret_robot_id(i) for i in robot_id]
        elif type(robot_id) == str:
...
def unittest():
    # random world
    grid_map, robot_char_1, robot_char_2 = random_world(10, 10, 5, 10)
    laserstr = make_laser_sensor(90, (1, 4), 0.5, False)
    proxstr = make_proximity_sensor(2, False)
    problem = MosOOPOMDP(robot_char_1,  # r is the robot character
